...
agg_file <- "/fh/fast/matsen_e/kdavidse/data/dnnir/Ab-VAE/vampire/all_TCRB_KD_cut.tsv"
igor_agg_file <- "/home/bolson2/Code/igor/agg_wd/agg_generated/generated_seqs_werr_CDR3_info.csv"
vae_agg_file <- "/fh/fast/matsen_e/kdavidse/data/dnnir/Ab-VAE/vampire/all_TCRB_KD_cut_predictions.tsv"
cmv_file <- "/fh/fast/matsen_e/kdavidse/data/dnnir/Ab-VAE/vampire/HIP13427_KD_cut.tsv"
igor_cmv_file <- "/home/bolson2/Code/igor/cmv_wd/cmv_generated/generated_seqs_werr_CDR3_info.csv"
vae_cmv_file <- "/fh/fast/matsen_e/kdavidse/data/dnnir/Ab-VAE/vampire/all_TCRB_KD_cut_predictions_HIP13427.tsv"
...
getSummaryDat <- function(dat,
    ...
    print(dat_name)
    dat_summary <- dat %>% summary_function
...
getSequenceLengthDistribution <- function(dat) {
    lengths <- dat %$%
        cdr3_aa %>%
        sapply(nchar) %>%
...
compareSequenceLengthDistributions <- function(dat_a,
                                               dat_b) {
    dist_a <- dat_a %>% getSequenceLengthDistribution
    dist_b <- dat_b %>% getSequenceLengthDistribution
...
agg_table %>%
    setNames(c("Summary", "Div(Observed, Igor)", "Div(Observed, VAE)",
              "Divergence type")) %>%
